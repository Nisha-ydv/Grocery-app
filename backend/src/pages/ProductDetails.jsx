...
const ProductDetails = () => {
  const [thumbnail, setThumbnail] = useState(null);
    const {products,navigate,addToCart}=useAppContext(AppContext)
    const {id}=useParams();
                ...
                <div className="flex gap-3">
                    <div className="flex flex-col gap-3">
                        {product.image.map((image, index) => (
                            <div 
                            key={index} 
                            onClick={() => setThumbnail(image)} className="border max-w-24 border-gray-500/30 rounded overflow-hidden cursor-pointer" >
